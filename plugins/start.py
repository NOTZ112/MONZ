...
from helper_func import subscribed, encode, decode, get_messages, get_missing_channels
...
async def not_joined(client: Client, message: Message):
    ...
    buttons = []
    for i, channel_id in enumerate(missing_channels, start=1):
        link = client.invitelinks.get(channel_id)
        if link:
            buttons.append(
                [InlineKeyboardButton(text=f"Join Channel {i}", url=link)]
            )
...
async def delete_files(messages, client, k):
    ...
    await k.edit_text(" Msg me @Sanasxbot ✅")
